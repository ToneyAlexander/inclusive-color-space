function build_picker() {
    ...
    let stored_x = canvas.width / 2.0
    let stored_y = canvas.height / 2.0
    let stored_z = 0
    ...
    function refreshUI() {
        let w_mid = canvas.width / 2.0
        let h_mid = canvas.height / 2.0
        cursor.style.left = `${w_mid}px`;
        cursor.style.top = `${h_mid}px`;
...
window.addEventListener("resize", () => {
    ...
    pickerTimer = setTimeout(build_picker, 100);
});
